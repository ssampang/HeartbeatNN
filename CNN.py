...
def build_cnn(input_var=None):
	...
	addLayer(network, lasagne.layers.DenseLayer(lasagne.layers.dropout(network['MaxPool1'], p=0.2), num_units=5), 'FC1')
	addLayer(network, lasagne.layers.DenseLayer(lasagne.layers.dropout(network['FC1'], p=0.2), num_units=5), 'FC2')
...
def modify_cnn(network):
	addLayer(network, lasagne.layers.DenseLayer(lasagne.layers.dropout(network['FC1'], p=0.2), num_units=100), 'FC2')
	addLayer(network, lasagne.layers.DenseLayer(network['FC2'],num_units=1,nonlinearity = lasagne.nonlinearities.sigmoid),'Sigmoid')
	return (network, network['Sigmoid'], [network['Conv1'], network['FC1'], network['FC2']])
